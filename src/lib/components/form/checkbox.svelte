<script lang="ts">
	import { AppState, FormConfig, setCollection } from "$lib";
    import type { BudgetCategory, Budget } from "$lib";

    export let value: BudgetCategory;
    export let fcindex: string;
    export let srindex: string;

    let style: string;
    let set: BudgetCategory[];

    $: fc = $FormConfig.collection as Budget
    $: set = fc[fcindex] as BudgetCategory[]
    ...
    function prechecked(items: BudgetCategory[]){
        let q = false
        let count = 1
    ...
    $: if (prechecked(set)){
        style = `border: 2px solid ${$AppState.color}; background: ${$AppState.color}4d;`;
    } else {
        style = `border: 2px solid ${$AppState.color}; background: #ffffff00;`;
    };
</script>

<button {style} class="Checkbox" on:click="{toggleChecked}">
{#if prechecked(set)}
